export function stableFingerprint(value: string): string {
  return ["", "appearance:", "legacy:", "hearthmere:"].map((salt) => fnv1a64(`${salt}${value}`)).join("");
}

/** Browser-safe SHA-256 for provenance and security-sensitive content identity. */
export function sha256Fingerprint(value: string): string {
  const bytes = utf8Bytes(value);
  const bitLength = BigInt(bytes.length) * 8n;
  const padded = [...bytes, 0x80];
  while (padded.length % 64 !== 56) padded.push(0);
  for (let shift = 56n; shift >= 0n; shift -= 8n) padded.push(Number(bitLength >> shift & 0xffn));
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const sigma0 = (rotateRight(left, 7) ^ rotateRight(left, 18) ^ left >>> 3) >>> 0;
      const sigma1 = (rotateRight(right, 17) ^ rotateRight(right, 19) ^ right >>> 10) >>> 0;
...
const rotateRight = (value: number, count: number): number => (value >>> count | value << 32 - count) >>> 0;
...
function fnv1a64(value: string): string {
  ...
  for (const byte of utf8Bytes(value)) { hash ^= BigInt(byte); hash = BigInt.asUintN(64, hash * 0x100000001b3n); }
