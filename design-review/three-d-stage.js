...
(() => {
  const stylesheet = `
    :host {
      ...
      width: 100%;
      height: 100vh;
      background: var(--stage-bg, #0b1113);
      overflow: hidden;
    }
    ...
    .toolbar button:hover { color: #e4c77e; border-color: #b99552; background: rgba(5, 8, 9, 0.85); }
  ...
  class ThreeDStage extends HTMLElement {
    ...
    async _boot() {
      ...
      this._fill = fill;
      scene.add(fill);
      this.setLightMode(this.getAttribute('light') || 'studio');
    ...
    /** Swap the studio rig for a region-graded rig. 'region' is the
     *  Hearthmere lighting recipe declared in src/data/worldAssets.js —
     *  cold_overcast key, warm_spring_bounce fill, contrast 0.62. 'studio'
     *  is the neutral rig for judging form and material. */
    setLightMode(mode) {
      ...
      if (mode === 'region') {
        hemi.color.set('#93a9ae');
        hemi.groundColor.set('#4a3a2c');
        hemi.intensity = 0.72;
        key.color.set('#c4d1d4');
        key.intensity = 1.62;
        key.position.set(3.2, 7.6, 4.4);
        fill.color.set('#c9793c');
        fill.intensity = 1.12;
        fill.position.set(-4, 1.4, -3.2);
        this.style.setProperty('--stage-bg', '#0b1113');
      } else {
        hemi.color.set(0xffffff);
        hemi.groundColor.set(0xd8d2c4);
        hemi.intensity = 1.0;
        key.color.set(0xffffff);
        key.intensity = 2.2;
        key.position.set(4, 7, 5);
        fill.color.set(0xfff4e6);
        fill.intensity = 0.5;
        fill.position.set(-5, 3, -4);
        this.style.setProperty('--stage-bg', '#171f21');
      }
      this.lightMode = mode;
    }
    ...
    setObject(object) {
      ...
      object.traverse((o) => {
        if (o.isMesh) {
          // A single-surface mesh (cloth, foliage card, decal plate) that
          // casts into its own shadow map is pure acne — let a model opt out
          // per mesh via userData.
          o.castShadow = o.userData.noCast !== true;
          o.receiveShadow = o.userData.noShadow !== true;
        }
      });
