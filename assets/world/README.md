...
All twelve accepted world concepts are direction and blockout references. The five regional references are also used as legacy 2D runtime backdrops; the Cathedral, both Warden Reed concepts, and all four supplemental Hollow Abbey concepts remain direction-only. The separate Veil technical reference is topology-only, and the Wave 03 machine references remain independently reviewed and noncanonical. None claims final coordinates, collision, navigation, meshes, animation, LOD, construction validity, or production readiness.

Public art provenance is split by release scope: `world-environments.current.batch-01.provenance.json` covers Graven March and the Cathedral; `world-environments.current.batch-02.provenance.json` binds the four legacy keyframes and both Warden Reed concepts; `world-environments.current.batch-03.provenance.json` binds the first two supplemental Hollow Abbey concepts; `world-environments.current.batch-04a.provenance.json` binds the Rain Court work nexus; and `world-environments.current.batch-04b.provenance.json` binds the Foundry operational chain. Exact accepted Hollow Abbey revision prompts are content-addressed in the matching batch-03, batch-04a, and batch-04b prompt packets; no execution locator, superseded private input locator, or private session metadata is published.
